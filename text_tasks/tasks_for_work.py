...
def input_parameters_work(i=None):
    ...
    values = {
        ...
        'pers3': ["студентка Кристина", "Фрекен Бок", "дровосек Саша", "блогер Лева", "Вася", "певица Настя",
                  ...
                  "предприятие"],
        'task': [[("изготовить образец для измерений", "шт"), ("съесть пиццу", 'шт'),
                  ("провести измерения", None), ("решить задачу", "шт"), ("выплавить сплав", None),
                  ('отшлифовать образцы', "шт")],
                 [("распугать воров", None), ("сварить варенье", "шт"),
                  ("съесть торт", "шт"), ("съесть плюшки", "шт")],
                 [("снять обзор на топоры", "ч"), ("наколоть дров", "шт"),
                  ("срубить дерево", "шт"), ("донести хворост", None)],
                 [(f'снять видео {choice(["в TikTok", "на YouTube", "в Инстаграм"])}', "шт"),
                  ('написать текст для публикации', "шт"),
                  (f'снять обзор на {choice(["машину", "ресторан", "игру", "квартиру"])}', "шт"),
                  ("снять видео", "шт")],
                 [("прополоть грядку", "шт"), ("покрасить забор", "шт"),
                  ("отремонтировать машину", "шт")],
                 [("сыграть концерт", "шт"), ("сочинить песню", "шт"),
                  ("выступить на фестивале", None), ("сочинить к музыке стихи", None)],
                 [("вылечить пациента", 'чел'), ("поставить укол", "шт"),
                  ("смешать физраствор", None), ("сменить повязку больному", None),
                  ("сделать перевязку", "шт")],
                 [("съесть овцу", "шт"), ("съесть курицу", "шт")],
                 [("покрасить забор", "шт"), ("выполнить заказ", "шт"),
                  ("починить санузел", "шт")],
                 [("выточить деталь", "шт"), ("починить станок", "шт"),
                  ("обработать деталь", "шт"), ("заточить сверло", None)],
                 [("сшить кофту", "шт"), ("сшить костюм на хэллоуин", None),
                  ("сшить костюм на ComicCon", None), ("сшить костюм для кота", None)],
                 [("наполнить бассейн", 'м3')],
                 [('выпустить изделие', "шт"), ('изготовить изделие', "шт")],
                 [("изготовить насос", "шт"), ('выпустить изделие', "шт")],
                 ]
    }
...
def task_17622():
    ...
    while True:
        pers1, pers2, _, (task, unit) = input_parameters_work()
        if unit:
            break
        else:
