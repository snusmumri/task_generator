...
def input_parameters_work(i=None):
    ...
    i = i if i is not None else randint(0, 13)

    values = {
        'pers1': ["студент Саша", "Карлсон", 'дровосек Миша', "блогер Коля", "Федя", "певец Паша", "медбрат Вова",
                  "лев", "первый рабочий", "токарь Миша", "швея Аня", "первая труба", "цех", "предприятие"],
        'pers2': ["студент Петя", "Малыш", "дровосек Коля", "блогер Толя", "Паша", "певец Саша", "медбрат Рома", "волк",
                  "второй рабочий", "токарь Вася", "швея Оля", "вторая труба", "цех", "предприятие"],
        'pers3': ["студентка Кристина", "Фрекен Бок", "дровосек Саша", "блогер Лева", "Вася", "певица Настя",
                  ...
                  "предприятие"],
        'task': [[("изготовить образец для измерений", ('в количестве', "шт")), ("съесть пиццу", ('размером', 'м2')),
                  ("провести измерения", None), ("решить задачу", ('в количестве', "шт")), ("выплавить сплав", None),
                  ('отшлифовать образцы', ('в количестве', "шт"))],
                 [("распугать воров", None), ("сварить варенье", ('в количестве', "шт")),
                  ("съесть торт", ('объемом', 'шт')), ("съесть плюшки", ('в количестве', "шт"))],
                 [("снять обзор на топоры", ('продолжительностью', 'ч')), ("наколоть дров", ('в количестве', "шт")),
                  ("срубить дерево", ('в количестве', "шт")), ("донести хворост", None)],
                 [(f'снять видео {choice(["в TikTok", "на YouTube", "в Инстаграм"])}', ('кол', 'шт')),
                  ('написать для публикации текст', None),
                  (f'снять обзор на {choice(["машину", "ресторан", "игру", "квартиру"])}', ('кол', 'шт')),
                  ("снять видео", ('кол', 'шт'))],
                 [("прополоть грядку", ('кол', 'шт')), ("покрасить забор", ('кол', 'шт')),
                  ("отремонтировать машину", ('кол', 'шт'))],
                 [("сыграть концерт", ('кол', 'шт')), ("сочинить песню", ('кол', 'шт')),
                  ("выступить на фестивале", None), ("сочинить к музыке стихи", None)],
                 [("вылечить пациента", ('кол', 'чел')), ("поставить укол", ('кол', 'шт')),
                  ("смешать физраствор", None), ("сменить повязку больному", None),
                  ("сделать перевязку", ('кол', 'шт'))],
                 [("съесть овцу", ('кол', 'шт')), ("съесть курицу", ('кол', 'шт'))],
                 [("покрасить забор", ('кол', 'шт')), ("выполнить заказ", ('кол', 'шт')),
                  ("починить санузел", ('кол', 'шт'))],
                 [("выточить деталь", ('кол', 'шт')), ("починить станок", ('кол', 'шт')),
                  ("обработать деталь", ('кол', 'шт')), ("заточить сверло", None)],
                 [("сшить кофту", ('кол', 'шт')), ("сшить костюм на хэллоуин", None),
                  ("сшить костюм на ComicCon", None), ("сшить костюм для кота", None)],
...
def task_9515():
    ...
    pers1, pers2, pers3, (task, _) = input_parameters_work()
...
def task_9517():
    """Генерация аналогичной задачи с портала https://kuzovkin.info/one_exercise_1/9517:
     ...
     """
    # получаем сюжет задачи
    pers1, pers2, _, (task, _) = input_parameters_work()
    while True:
...
def task_17622():
    ...
    if unit in ('м3', 'м2'):
        ...
        new_unit = MorphAnalyzer().parse(task.split()[1])[0].inflect({'accs', 'sing'}).word

    if pers1 == pers2:
        pers1_gent = MorphAnalyzer().parse(pers1)[0].inflect({'gent'}).word
        return v1, f"Два {pers1_gent} могут {task1} {v} {word1}. " \
                   f"При этом первый {pers1} работает {t1} ч, а второй {pers2} - {t2} ч. " \
                   f"Какова производительность первого {pers1_gent}, если 1 {new_unit} он может {task.split()[0]} на {t} мин медленнее, чем второй?"""

    else:
        if len(pers1.split()) == 1:
            pers1_gent = MorphAnalyzer().parse(pers1)[0].inflect({'gent'}).word
        else:
            pers1_gent_1, pers1_gent_2 = (MorphAnalyzer().parse(item)[0].inflect({'gent'}).word for item in pers1.split())
            pers1_gent = f'{pers1_gent_1} {pers1_gent_2}'
        # task_word = MorphAnalyzer().parse(task.split()[1])[0].inflect({'accs'}).word
