...
from .function_for_task import choosing_declension_form, capitalize_word, find_number_object, find_genus_object, \
    write_numeral_word


def generate_context(i=0):
  '''Функция содержит три списка возможных вариантов сюжета для задач,
  подбирает случаное значение из списка и выдает ТРИ случайных парамета item1, item2, task'''
  values = {
      ...
      'task': [[('морская вода', 'соль', 'пресная вода'),
                ...
                ('гранатовый сок', 'гранатовая эссенция', 'вода'),
                ('уксусная кислота', 'уксус', 'чистая вода'),
                ('целлюлозная масса', 'древесина', 'вода'),
...
def task_2614():
    ...
    item1, item2, _ = generate_context()
    while True:
        ...
        if (2 * weight + weight1 - weight1 * 100 / percent) ** 2 - 4 * weight * (weight + weight1) == 0 and 0 < result < 1000 and result * 1000 % 1000 == 0:
            answer = int(result)
            break
    task = f'В сплав {choosing_declension_form(item1)} с {choosing_declension_form(item2, "ablt")}, содержащий {weight} г {choosing_declension_form(item1)}, добавили {weight1} г {choosing_declension_form(item1)}. В результате содержание {choosing_declension_form(item1)} в сплаве увеличилось на {percent} %. Сколько граммов {choosing_declension_form(item2)} в сплаве?'
    return task, answer
...
def task_4767():
    ...
    while True:
        ...
        if result * 1000 % 1000 == 0 and percent1 > percent2:
            answer = int(result)
            break
    task = f'{capitalize_word(choosing_declension_form(param[0], "nomn"))} содержит {percent1}% (по весу) {choosing_declension_form(param[1])}. Сколько килограммов {choosing_declension_form(param[2])} нужно добавить к {weight} кг {choosing_declension_form(param[0])}, чтобы содержание {choosing_declension_form(param[1])} в {choosing_declension_form(param[0], "loct")} составило {percent2}%?'
    return task, answer
...
def task_4770():
    ...
    while True:
        ...
        if percent2 < percent1 and result * 1000 % 1000 == 0:
            answer = int(result)
            break
    task = f'Сколько килограммов {choosing_declension_form(param[2])} нужно выпарить из {weight/1000} т {choosing_declension_form(param[0])}, содержащей {percent1}% {choosing_declension_form(param[2])}, чтобы получить {choosing_declension_form((param[0] if len(param[0].split())<2 else param[0].split(" ", 1)[1]), "accs")} с содержанием {percent2}% {choosing_declension_form(param[2])}?'
    return task, answer
...
def task_4772():
    ...
    while True:
        ...
        if percent2 < (percent1 - 40) and result * 1000 % 1000 == 0:
            ...
            answer = result
            break
    task = f'{capitalize_word(param[0])} содержат по весу {percent1}% {choosing_declension_form(param[2])}, а {param[1]} − {percent2}% {choosing_declension_form(param[2])}. Сколько получится {choosing_declension_form(param[1])} из {weight1} кг {choosing_declension_form(param[0].split()[0])}?'
    return task, answer
...
def task_4773():
    ...
    while True:
        ...
        if percent1 < (percent2 - 10) and result * 1000 % 1000 == 0:
            ...
            answer = result
            break
    task = f'Кусок сплава {choosing_declension_form(item1)} и {choosing_declension_form(item2)} массой {weight} кг содержит {percent1}% {choosing_declension_form(item1)}. Какую массу {choosing_declension_form(item1)} следует добавить к этому куску, чтобы получить сплав, содержащий {percent2}% {choosing_declension_form(item1)}?'
    return task, answer
...
def task_4774():
    ...
    while True:
        ...
        if weight1 < weight2 and result * 1000 % 1000 == 0:
            ...
            answer = result
            break
    task = f'В {weight1} литра {percent}-процентного раствора {choosing_declension_form(param[0])} добавили {weight2} л {choosing_declension_form(param[1])}. Определите процентное содержание {choosing_declension_form(param[0])} в полученном растворе.'
    return task, answer
...
def task_4778():
    ...
    while True:
        ...
        if percent / num * 10000 % 100 == 0:
          answer = result
          break
    task = f'В {weight} кг сплава {choosing_declension_form(item1)} и {choosing_declension_form(item2)} содержится {percent}% {choosing_declension_form(item2)}. Сколько кг {choosing_declension_form(item1)} надо добавить к сплаву, чтобы процентное содержание {choosing_declension_form(item2)} стало {write_numeral_word(num)} меньше?'
    return task, answer
...
def task_4780():
    ...
    while True:
        ...
        if percent1 > percent2 and result * 1000 % 1000 == 0:
            answer = int(result)
            break
    task = f'{capitalize_word(param[0])} содержит {int(percent1) if percent1 * 10 % 10 == 0 else percent1}% {choosing_declension_form(param[1])} по массе. Сколько килограммов {choosing_declension_form(param[2])} нужно добавить к {weight} кг {choosing_declension_form(param[0])}, чтобы {"она" if find_genus_object(param[0]) == 2 else "он"} {"содержала" if find_genus_object(param[0]) == 2 else "содержал"} {int(percent1) if percent2 * 10 % 10 == 0 else percent2}% {choosing_declension_form(param[1])} по массе?'
    return task, answer
...
def task_4782():
    ...
    while True:
        ...
        if weight2 < weight1 and result * 1000 % 1000 == 0:
            answer = int(result)
            break
    task = f'Из {int(weight1) if weight1 * 10 % 10 == 0 else weight1} кг {choosing_declension_form(param[0])} получают {int(weight2) if weight2 * 10 % 10 == 0 else weight2} кг {choosing_declension_form(param[1])}, {"содержащих" if find_number_object(param[0]) == 2 else "сoдержащего"} {percent}% {choosing_declension_form(param[2])}. Каково процентное содержание {choosing_declension_form(param[2])} в {choosing_declension_form(param[0], "loct")}?'
    return task, answer
...
def task_4787():
    ...
    while True:
        ...
        if percent1 > percent2 and result * 1000 % 1000 == 0:
            ...
            answer = result
            break
    task = f'Какое количество {percent1}%-го раствора {choosing_declension_form(param[0])} надо взять, чтобы его можно было развести {choosing_declension_form(param[1], "ablt")} до получения {weight} г {percent2}%-го раствора {choosing_declension_form(param[0])}?'
    return task, answer
...
def task_4788():
    ...
    while True:
        ...
        if percent1 < percent2 and result * 1000 % 1000 == 0:
            answer = int(result)
            break
    task = f'{capitalize_word(param[0])} содержит {percent1}% {choosing_declension_form(param[1])}. Сколько килограммов {choosing_declension_form(param[2].split()[-1])} надо выпарить из {weight} кг {choosing_declension_form(param[0])}, чтобы концентрация {choosing_declension_form(param[1])} в {"ней" if find_genus_object(param[0]) == 2 else "нём"} увеличилась до {percent2}%?'
    return task, answer
...
def task_4789():
    ...
    while True:
        ...
        if percent1 < percent2 and result * 1000 % 1000 == 0:
            answer = int(result)
            break
    task = f'Сколько {choosing_declension_form(param[1])} надо выпарить из {weight} г {percent1}%-го раствора {choosing_declension_form(param[0])}, чтобы получить {percent2}%-ый раствор?'
    return task, answer
    ...
    while True:
        ...
        if d > 0 and (d**0.5)*1000%1000 == 0:
          ...
          if x1*1000%1000 == 0 and x2*1000%1000 == 0 and (x1-weight1) > weight1 and (x2-weight1) <=weight1:
            answer = int(x1)
            break
    task = f'В сплав {choosing_declension_form(item1)} и {choosing_declension_form(item2)}, содержащий {weight1} кг {choosing_declension_form(item2)}, добавили {weight2} кг {choosing_declension_form(item2)}, после чего содержание {choosing_declension_form(item2)} в сплаве повысилось на {percent} %. Какова первоначальная масса сплава, если известно, что в нем {choosing_declension_form(item1)} было больше, чем {choosing_declension_form(item2)}?'
    return task, answer
...
def task_6489():
    ...
    while True:
        ...
        if percent1 < percent2 and result*10000%10000 == 0:
          ...
          answer = result
          break
    task = f'Cлиток сплава {choosing_declension_form(item1)} и {choosing_declension_form(item2)} массой {weight} кг содержит {percent1}% {choosing_declension_form(item1)}. Какую массу {choosing_declension_form(item1)} надо добавить к этому куску, чтобы плученный сплав содержал {percent2}% {choosing_declension_form(item1)}?'
    return task, answer
...
def task_9479():
    ...
    while True:
        ...
        if percent1 > percent2 and result*10000%10000 == 0:
          ...
          answer = result
          break
    task = f'Сплав {choosing_declension_form(item1)} с {choosing_declension_form(item2, "ablt")} весом {weight} кг содержит {percent1}% {choosing_declension_form(item2)}. Сколько {"чистой" if find_genus_object(item1) == 2 else "чистого"} {choosing_declension_form(item1)} надо добавить, чтобы получить сплав, содержащий {percent2}% {choosing_declension_form(item2)}?'
    return task, answer
...
def task_9481():
    ...
    while True:
        ...
        if percent1 > percent2+30 and result*10000%10000 == 0:
          answer = int(result)
          break
    task = f'{capitalize_word(param[0])} {"содержит" if find_number_object(param[0]) == 1 else "содержат"} по массе {percent1}% {choosing_declension_form(param[2])}, а {"сухой" if find_number_object(param[0]) == 1 else "сухие"} − {percent2}%. Сколько надо собрать {choosing_declension_form(param[0])}, чтобы из {"него" if find_number_object(param[0]) == 1 else "них"} получить {weight} кг {"сухого" if find_number_object(param[0]) == 1 else "сухих"} {choosing_declension_form(param[0].split()[1])}?'
    return task, answer
...
def task_9486():
    ...
    while True:
        ...
        if percent1 < percent2 and result*10000%10000 == 0:
          answer = int(result)
          break
    task = f'В {weight} кг сплава {choosing_declension_form(item1)} и {choosing_declension_form(item2)} содержится {percent1}% {choosing_declension_form(item2)}. Сколько кг {choosing_declension_form(item2)} надо добавить к этому сплаву, чтобы его процентное содержание в новом сплаве стало равно {percent2}%?'
    return task, answer
    ...
    while True:
        ...
        if (percent1+10) < percent2:
            ...
            if result*10000%10000 == 0:
              answer = int(result)
              break
    task = f'К {percent1}% раствору {choosing_declension_form(param[0])} добавили {weight} г {"чистой" if find_genus_object(param[0]) == 2 else "чистого"} {choosing_declension_form(param[0])}, после чего концентрация раствора стала равна {percent2}%. Найдите первоначальный вес раствора.'
    return task, answer
...
def task_9491():
    ...
    while True:
        ...
        if percent1 > percent2 and result*10000%10000 == 0:
              answer = int(result)
              break
    task = f'Собрали {weight} кг {choosing_declension_form(param[0].split()[1])}. Оказалось, что {"их" if find_number_object(param[0]) == 2 else "его"} влажность {percent1}%. Когда {"их" if find_number_object(param[0]) == 2 else "его"} подсушили, то влажность снизилась до {percent2}%. Какой стала масса {"этих" if find_number_object(param[0]) == 2 else "этого"} {choosing_declension_form(param[0].split()[1])} после того, как {"их" if find_number_object(param[0]) == 2 else "его"} подсушили?'
    return task, answer
...
def task_9495():
    ...
    while True:
        ...
        if percent1+10 < percent2 and result*10000%10000 == 0:
              answer = int(result)
              break
    task = f'Только что добыт{"ая" if find_genus_object(param[0]) == 2 else "ый"} {param[0]} содержит {percent1}% воды, а после двухнедельного пребывания на воздухе {"она" if find_genus_object(param[0]) == 2 else "он"} содержит {percent2}% {find_genus_object(param[1])}. На сколько кг увеличилась масса добытой {weight//1000 if weight > 1000 else ""}{" " if weight > 1000 else ""}тонн{"ы" if weight//1000 in [1,2,3,4] else ""} {weight%1000 if weight%1000 != 0 else ""}{" колограмма " if weight%1000 in [2, 3, 4] else " колограммов " if weight%1000 != 0 else ""}{find_genus_object(param[0].split()[1] if len(param[0].split()) > 1 else param[0])} после того, как {"она" if find_genus_object(param[0]) == 2 else "он"} две недели полежал{"а" if find_genus_object(param[0]) == 2 else ""} на воздухе?'
    return task, answer
...
def task_9496():
    ...
    while True:
        ...
        if percent1 > percent2+50 and result*10000%10000 == 0:
              answer = int(result)
              break
    task = f'{"Виноград" if param[0].split()[1] == "виноград" else capitalize_word(param[0])} содерж{"ит" if find_number_object(param[1]) == 1 else "ат"} {percent1}% влаги, а {"изюм" if param[1].split()[1] == "виноград" else param[1]} − {percent2}%. Сколько килограммов {"винограда" if param[0].split()[1] == "виноград" else choosing_declension_form(param[0])} требуется для получения {weight} килограмм{"а" if weight%10 == 1 else "ов"} {"изюма" if param[1].split()[1] == "виноград" else choosing_declension_form(param[1])}?'
    return task, answer
    ...
    while True:
        ...
        if weight1 > weight2:
          ...
          if result > 0 and result*10000%10000 == 0 and (weight2 * result/100)*1000%100 == 0:
                answer = int(result)
                break
    task = f'Из {weight1} т сырья {param[1]} сорта, содержащего {percent}% примесей, после очистки получается {weight2} т сырья {param[0]} сорта. Каков процент примесей в сырье {param[0]} сорта?'
    return task, answer
    ...
    while True:
         ...
         if percent1+10 < percent2:
           ...
           if result*10000%10000 == 0:
                 answer = int(result)
                 break
    task = f'Сколько килограммов {choosing_declension_form(param[1])} с влажностью {percent1}% можно получить из {weight} кг {choosing_declension_form(param[0])} с влажностью {percent2}%?'
    return task, answer
    ...
    while True:
         ...
         if percent1 + percent2 > 100:
           ...
           if result*10000%10000 == 0:
                 answer = int(result)
                 break
    task = f'Имеется {weight} г сплава, содержащего {item1} и {item2}. {capitalize_word(item1)} составляет {percent1}% сплава. Сколько граммов {choosing_declension_form(item2)} надо добавить к этому сплаву, чтобы новый сплав содержал {percent2}% {choosing_declension_form(item2)}?'
    return task, answer
...
def task_11070():
    ...
    while True:
      ...
      if result*10000%10000 == 0:
        answer = int(result)
        break
    task = f'К {weight1} кг сплава {choosing_declension_form(item1)} и {choosing_declension_form(item2)} добавили {weight2} кг {choosing_declension_form(item1)}. Найдите процентное содержание {choosing_declension_form(item2)} в первоначальном сплаве, если в новом сплаве {choosing_declension_form(item2)} стало в {multipl} раза меньше, чем {choosing_declension_form(item1)}.'
    return task, answer
    ...
    while True:
         ...
         if weight1 > weight2+20:
           ...
           if result > 0 and result*10000%10000 == 0:
                 answer = int(result)
                 break
    task = f'Собрали {weight1} кг {choosing_declension_form(param[0])}, содержащ{"их" if find_number_object(param[0]) == 2 else "его"} по массе {percent}% {choosing_declension_form(param[2])}. Когда {"их" if find_number_object(param[0]) == 2 else "его"} подсушили, {"они" if find_number_object(param[0]) == 2 else "он"} стал{"и" if find_number_object(param[0]) == 2 else ""} весить {weight2} кг. Каков процент содержания {choosing_declension_form(param[2])} по массе в {choosing_declension_form(param[1], "loct")}?'
    return task, answer
    ...
    while True:
        ...
        if percent1 > percent2:
            ...
            if result * 1000 % 1000 == 0:
              answer = int(result)
              break
    task = f'{capitalize_word(param[0])} содержит {percent1}% {choosing_declension_form(param[1])}. Сколько килограммов {choosing_declension_form(param[2])} нужно добавить к {weight} кг {choosing_declension_form(param[0])}, чтобы содержание {choosing_declension_form(param[1])} в смеси составило {percent2}%?'
    return task, answer
    ...
    while True:
        ...
        if weight1 < weight2:
            ...
            if result * 1000 % 1000 == 0:
              answer = int(result)
              break
    task = f'В {weight1} литр{("а" if weight1 in [2, 3, 4]  else "ов") if weight1 != 1 else "" } {percent}-процентного раствора {choosing_declension_form(param[0])} добавили {weight2}л {choosing_declension_form(param[1])}. Определить процентное соотношение {choosing_declension_form(param[0])} в полученном растворе.'
    return task, answer